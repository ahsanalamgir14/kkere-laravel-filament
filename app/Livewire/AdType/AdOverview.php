...
class AdOverview extends Component implements HasForms
{
    ...
    protected function setDescriptionHtml()
    {
        ...
        $this->descriptionHtml = tiptap_converter()->asHTML($this->ad->description_tiptap);
    }
    ...
    protected function buildBreadcrumbs()
    {
        ...
        if ($this->ad->category || $this->ad->mainCategory) {
            $subCategory = $this->ad->category;
            $category = $this->ad->category->parent ?? $this->ad->mainCategory;
        ...
        if ($this->ad->childCategory) {
            $url = $this->getChildCategoryUrl($this->ad->childCategory->id);
            $this->breadcrumbs[$url] = $this->ad->childCategory->name;
        }

        $this->breadcrumbs[] = $this->ad->title;
    }
    /**
    ...
    public function addToCart()
    {
        ...
        if (auth()->check()) {
            ...
            if ($cart) {
                ...
                if (isECommerceQuantityOptionEnabled() && $cart->quantity >= getECommerceMaximumQuantityPerItem()) {
                    Notification::make()
                ...
                $finalQuantity = $totalQuantity >= $maxQuantity ? $maxQuantity : $totalQuantity;
            ...
            foreach ($cart as &$item) {
                if ($item['ad_id'] == $this->ad->id) {
                    $newQuantity = $item['quantity'] + $this->cartQuantity;

                    if (isECommerceQuantityOptionEnabled() && $newQuantity > getECommerceMaximumQuantityPerItem()) {
                        Notification::make()
                            ->title(__('messages.t_cart_max_limit_reached', ['max' => getECommerceMaximumQuantityPerItem()]))
                            ->warning()
                            ->send();

                        $item['quantity'] = getECommerceMaximumQuantityPerItem();
                    } else {
    ...
    private function setMaxQuantityValue()
    {
        ...
        if (isECommerceQuantityOptionEnabled() && $this->cartQuantity > getECommerceMaximumQuantityPerItem()) {
    ...
    {
        $this->categorySlug = Category::find(
            $this->ad->main_category_id ?? $this->ad->category?->parent_id
        )?->slug;
        $this->subCategorySlug = Category::find($this->ad->category_id)?->slug;
        $this->childCategorySlug = Category::find($this->ad->child_category_id)?->slug;
    }
    ...
    {
        return view('livewire.ad-type.ad-details.ad-overview');
    }
}
