<a href="{{ route('ad.overview', ['slug' => $ad->slug, 'ref' => $ref]) }}" aria-label="{{ $ad->title }}"
    class="w-full shadow-sm flex-none bg-white rounded-xl hover:shadow-md  dark:bg-gray-900 classic:border-black classic:border {{ $homeSettings->enable_hover_animation ? 'classic-hover-shadow' : '' }} relative flex flex-col md:pb-0 pb-[2.5rem] border dark:border-white/10"
    aria-label="{{ $ad->title }}" wire:click="saveClicks">
