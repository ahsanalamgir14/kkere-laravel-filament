<a href="{{ route('ad.overview', ['slug' => $ad->slug, 'ref' => $ref]) }}" aria-label="{{ $ad->title }}"
    wire:click="saveClicks"
    class=" rounded-xl hover:shadow-md dark:bg-gray-900 border dark:border-white/10 classic:border-black {{ $homeSettings->enable_hover_animation ? 'classic-hover-shadow' : '' }} relative bg-white shadow-sm">
        ...
        <div class="pb-0 p-2 md:p-3  md:pb-0 relative  md:w-auto md:h-auto flex-none">
            ...
            @if ($isUrgent)
                ...
                theme: $store.theme,
                    ...
                    @if ($urgentAdColors) style="{{ $urgentAdColors->background_color ? 'background:' . $urgentAdColors->background_color : '#DC2626' }} ;{{ $urgentAdColors->text_color ? 'color:' . $urgentAdColors->text_color : '#000000;' }}" @endif>
            ...
            @if ($isFeatured)
                ...
                theme: $store.theme,
                    ...
                    @if ($featureAdColors) style="{{ $featureAdColors->background_color ? 'background:' . $featureAdColors->background_color : '#FACC15' }}; {{ $featureAdColors->text_color ? 'color:' . $featureAdColors->text_color : '#000000;' }}" @endif>
            ...
            <img src="{{ $ad->primaryImage ?? asset('/images/placeholder.jpg') }}" alt="{{ $altText }}"
                class="aspect-square object-cover h-32 flex w-full md:h-[12rem] rounded-xl image-render-optimized"
        ...
        <div class="flex-grow flex flex-col">
            <div class="flex-grow  border-gray-200  dark:border-white/10 classic:border-black mt-auto">
                <div class="px-2 md:px-3 py-3 h-full flex flex-col ">
                    <h3 class="mb-1 text-sm md:text-base line-clamp-{{ $adTemplateSettings->max_line }} font-semibold">
                        {{ $ad->title }}</h3>
                    ...
                    @if (is_ecommerce_active())
                        <div class="flex items-center mb-2 text-sm ">
                            <span class="font-medium ml-1 line-clamp-1">{{ $ad?->category?->name }}</span>
                        </div>
                    @endunless
                    @unless ($ad?->adType?->disable_location)
                        <div class="flex items-center mb-2 text-sm ">
                            <x-icon-pin-location class="w-4 h-4 dark:text-gray-500" />
                            <span class="font-medium ml-1 line-clamp-1">{{ $ad->location_name }}</span>
                        </div>
                    @endunless
                    <span
...
<div>
    @if (
        ($ad->adType?->disable_price_type != true || $isFavourited) &&
            !in_array($ad->adType?->marketplace, [ONLINE_SHOP_MARKETPLACE, POINT_SYSTEM_MARKETPLACE]))
        <div
            class=" rtl:justify-end rtl:scale-x-[-1] flex justify-between items-center px-3 py-2 md:py-3 mt-3 border-t border-gray-200  dark:border-white/10 classic:border-black">
            <div class="flex items-center gap-x-2 justify-between w-full">
                @if ($ad->adType?->disable_price_type != true)
                    <x-price value="{{ formatPriceWithCurrency($ad->price) }}" type_id="{{ $ad->price_type_id }}"
                        label="{{ $ad->priceType->label }}" has_prefix="{{ $ad?->adType?->has_price_suffix }}"
    ...
    @if (in_array($ad->adType?->marketplace, [ONLINE_SHOP_MARKETPLACE, POINT_SYSTEM_MARKETPLACE]))
        <div
            class="flex justify-between items-center px-3 py-2 md:py-3 mt-3 border-t border-gray-200  dark:border-white/10 classic:border-black">
            <div class="flex items-center gap-x-2 justify-between w-full">
                <x-price value="{{ formatPriceWithCurrency($ad->price) }}" type_id="{{ $ad->price_type_id }}"
                    label="{{ $ad->priceType->label }}" has_prefix="{{ $ad?->adType?->has_price_suffix }}"
                    price_suffix="{{ $ad->price_suffix }}" offer_enabled="{{ $ad->isEnabledOffer() }}"
