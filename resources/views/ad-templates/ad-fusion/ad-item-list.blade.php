<a href="{{ route('ad.overview', ['slug' => $ad->slug, 'ref' => $ref]) }}" aria-label="{{ $ad->title }}"
    class="w-full shadow-sm flex-none bg-white rounded-xl hover:shadow-md border dark:border-white/10 dark:bg-gray-900 classic:border-black classic:border {{ $homeSettings->enable_hover_animation ? 'classic-hover-shadow' : '' }} relative flex flex-col md:pb-0 "
    aria-label="{{ $ad->title }}">
    <div class="w-full flex md:pb-0">
        ...
        <div class="pb-0 p-2 md:p-3  md:pb-0 relative  md:w-auto md:h-auto flex-none">
            ...
            <img src="{{ $ad->primaryImage ?? asset('/images/placeholder.jpg') }}" alt="{{ $altText }}"
                class="aspect-square object-cover h-28 flex w-full md:h-[12rem] rounded-xl image-render-optimized"
                loading="lazy" decoding="async">
            <div class="bg-[#b0b0b085] flex gap-2 w-fit px-1 py-[0.10rem] items-center rounded-md absolute top-4 right-4 z-[1]"
                x-data
                x-tooltip="{
        ...
        <div class="flex-grow flex flex-col">
            <div class="flex-grow  border-gray-200  dark:border-white/10 classic:border-black mt-auto">
                <div class="px-2 md:px-3 py-3 h-full flex flex-col ">
                    <h3
                        class="mb-2 text-sm md:text-base line-clamp-{{ $adTemplateSettings->max_line }} break-all font-semibold">
                        {{ $ad->title }} </h3>
    ...
    @if (
        ...
        <div
            ...
            <div class="flex items-center gap-x-2 justify-between w-full">
                @if ($ad->adType?->disable_price_type != true)
                    ...
                    @include('ad-templates.ad-fusion.price', [
                        'value' =>
                            ...
                            ' ' .
                            \Number::format(floor($ad->price), locale: $paymentSettings->currency_locale),
                        'type_id' => $ad->price_type_id,
                        'label' => $ad->priceType->label,
                        'has_prefix' => $ad?->category?->has_price_suffix,
                        'price_suffix' => $ad->price_suffix,
                        'offer_enabled' => $ad->isEnabledOffer(),
                        'offer_price' => $ad->offer_price
                            ? config('app.currency_symbol') .
                                ' ' .
                                \Number::format(floor($ad->offer_price), locale: $paymentSettings->currency_locale)
                            : null,
                ...
                <div class="ml-auto">
                    <div @click.prevent="$wire.addToFavourites" class="cursor-pointer " x-data
                        x-tooltip="{
