...
<div class="w-full border-b dark:border-white/20 classic:border-black">
    ...
    <div role="group" wire:ignore
        class="swiper mySwiper md:h-[25rem] h-[10rem] md:rounded-ss-xl bg-white dark:bg-gray-900">
        ...
        <!-- Navigation Buttons -->
        <button type="button" aria-label="{{ __('messages.t_aria_label_previous_item') }}" x-data
            x-tooltip="{
        ...
        <button type="button" aria-label="{{ __('messages.t_aria_label_next_item') }}" x-data
            x-tooltip="{
        ...
        @if ($customizationSettings['enable_autoplay'])
            <button x-cloak id="autoplayButton"
                ...
                x-data="{ isPlaying: {{ $customizationSettings['enable_autoplay'] ? 'true' : 'false' }} }" @click="isPlaying = !isPlaying; updateautoPlayStatus()"
    ...
    <script>
        ...
        if (bannerSettings.enablePaginationCount) {
            swiperOptions.pagination.renderBullet = function(index, className) {
                ...
                const button = '<button ' + tooltip + ' type="button" class="' + className + '">' + (index + 1) + '/' +
                    itemCount + "</button>";
        ...
        const observer = new IntersectionObserver((entries, observer) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    Livewire.dispatch('update-banner-view', {
                        bannerId: bannerId
                    });
                }
            });
    ...
    @if (
        $customizationSettings['enable_price_below_image'] &&
            ($ad->adType?->disable_price_type == false ||
                in_array($ad->adType?->marketplace, [ONLINE_SHOP_MARKETPLACE, POINT_SYSTEM_MARKETPLACE])))
        <x-custom-price value="{{ formatPriceWithCurrency($ad->price) }}"
            type_id="{{ in_array($ad->adType?->marketplace, [ONLINE_SHOP_MARKETPLACE, POINT_SYSTEM_MARKETPLACE]) ? 1 : $ad->price_type_id }}"
